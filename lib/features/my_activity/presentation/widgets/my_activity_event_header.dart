...
class MyActivityEventHeader extends StatelessWidget {
  final GatheringEntity event;
  const MyActivityEventHeader({super.key, required this.event});
      ...
      child: ClipRRect(
        ...
        child: SizedBox(
          ...
          child: BlocBuilder<MyActivityCubit, MyActivityState>(
            builder: (_, state) {
              final cubit = context.read<MyActivityCubit>();

              final avatars = (state is MyActivityParticipantsLoadedState)
                  ? state.participants
                  : cubit.participants;
